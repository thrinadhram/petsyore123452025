...
public class Usertest {
	...
	void testGetuserByname() throws InterruptedException {
		...
		Response res = Userendpoints.Readuser(currentUsername);
		res.then().log().all();
		Assert.assertEquals(res.getStatusCode(), 200);
	...
	void test_update_user_name() throws InterruptedException {
		...
		userpayload.setEmail(faker.internet().safeEmailAddress());
			...
			Response res = Userendpoints.Updateuser(currentUsername,userpayload);
	...
	void testdelateuser_by_name() throws InterruptedException {
		...
		Thread.sleep(1000);
		Response res=Userendpoints.deleteuser(currentUsername);
		 res.then().log().all(); 
		Assert.assertEquals(res.getStatusCode(), 200);
